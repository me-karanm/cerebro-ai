...
import { Plus, Search, Filter, Bot, Play, Edit, Copy } from 'lucide-react';
...
const sampleAgents = [
  ...
  // Add more sample agents for pagination demo
  ...Array.from({ length: 15 }, (_, i) => ({
    id: `${i + 4}`,
    name: `Agent ${i + 4}`,
    description: `Sample agent ${i + 4} description`,
    status: i % 2 === 0 ? 'active' : 'draft',
    language: 'English',
    voice: i % 2 === 0 ? 'Professional Female' : 'Professional Male',
    conversations: Math.floor(Math.random() * 1000),
    successRate: Math.floor(Math.random() * 40) + 60,
    lastUpdated: `${Math.floor(Math.random() * 7) + 1} days ago`,
    persona: `Sample persona for agent ${i + 4}`,
  })),
];
